...
export const Carousel3D: React.FC<Carousel3DProps> = ({
  ...
  useEffect(() => {
    const init = () => {
      ...
      const elements = spinRef.current.querySelectorAll('img, video');
      elements.forEach((el, i) => {
        const element = el as HTMLElement;
        element.style.transform = `rotateY(${i * (360 / elements.length)}deg) translateZ(${radius}px)`;
        element.style.transition = 'transform 1s';
        element.style.transitionDelay = `${(elements.length - i) / 4}s`;
      });
    };
    ...
    const applyTransform = (obj: HTMLElement) => {
      ...
      obj.style.transform = `rotateX(${-tY.current}deg) rotateY(${tX.current}deg)`;
    };
    ...
    const handlePointerDown = (e: PointerEvent) => {
      ...
      const handlePointerUp = () => {
        ...
        timer.timer = setInterval(() => {
          ...
          if (dragRef.current) applyTransform(dragRef.current);
          playSpin(false);
          if (Math.abs(tX.current) < 0.5 && Math.abs(tY.current) < 0.5) {
            clearInterval(timer.timer);
            playSpin(true);
          }
        }, 17);
      ...
      <DragContainer ref={dragRef}>
        ...
        >
          {items.map((item, index) => (
            <ItemLink key={index} href={item.href || '#'}>
              {item.type === 'video' ? (
